    ...
    <div className='p-3 bg-gray-900 bg-opacity-80 gap-4 sticky top-0 header z-20 backdrop-blur-sm md:hidden'>
      ...
      <div className={`fixed p-4 top-50 left-0 h-screen w-full bg-slate-800 ${showSide ? "block" : "hidden"} `}>
        ...
        <nav className=' text-sky-200 text-opacity-100 text-sm font-mono tracking-wider'>
          ...
          {admin ? <NavLink to="https://admin.kingstonemovies.xyz/api/v1/admin" target='_blank'  className="py-2 flex place-content-center  items-center justify-start  gap-3 transition-all duration-150 ease-linear border-b-2 border-gray-500 border-opacity-15" onClick={setttingShowFalse}>
            <Admin size={23}/>
