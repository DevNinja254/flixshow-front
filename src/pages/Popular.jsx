...
const Popular = () => {
    ...
    const fetchItems = async (page) => {
      ...
      try {
        ...
        if (path.includes("popular")){
            ...
            setSearchr('popular')
            const cartegory = sessionStorage.getItem("store")
            setPartialData(JSON.parse(cartegory).slice(0, 13))
        }else {
            url = `/videoDetails/?cartegory=${path[2]}&ordering=-date_uploaded&page_size=1000`
            const cartegory = sessionStorage.getItem("cartegory")
            if (cartegory) {
              for (const cart of JSON.parse(cartegory)) {
                if (cart.cartName === path[2]) {
                  // console.log(cart)
                  setPartialData(cart.video_details)
                  setCount(cart.total_related_count)
                }
              }
            }
             setSearchr(path[2])
  ...
  // console.log(patialData)
  return (
    <div className={`${redirecto ? "opacity-80" : "opacity-100"} transition-all duration-500 ease-in-out`}>
      <Layout>
      ...
      {error ? <p className='bg-red-700 text-white fixed bottom-4 left-3 p-2 text-sm font-bold rounded-sm'>Loading {path} failed!</p> : null}
        <main className=' bg-black search'>
              <div className="mx-3 py-2 md:w-11/12 md:mx-auto lg:w-10/12 xl:w-4/5">
            <p className='text-white text-sm font-mono text-center py-3 capitalize'>{datas.length + patialData.length} Movies and Series Available.</p>
